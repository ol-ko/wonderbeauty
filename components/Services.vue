    ...
    <div v-for="service in services" :class="$style.service">
      <div :class="$style.img">
        <nuxt-link :to="localePath(`services/${slug(service.title)}`)">
          <img
            :srcset="
              `/images/services/${slug(service.title)}_888x599.jpg 888px,
               /images/services/${slug(service.title)}_552x372.jpg 552px,
               /images/services/${slug(service.title)}_444x299.jpg 444,
               /images/services/${slug(service.title)}_276x186.jpg 276px`
            "
            sizes="(max-width: 599px) 100vw, (max-width: 959px) 50vw, 200px"
            :src="`/images/services/${slug(service.title)}_552x372.jpg`"
            :alt="service"
          />
        </nuxt-link>
      </div>
